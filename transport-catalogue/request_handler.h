...
public:
    ...
    // Методы получения информации по определённому запросу
    ...
    std::pair<std::vector<transport_catalogue::BusPtr>, std::vector<transport_catalogue::StopPtr>> GetAllBusesAndStops(const std::vector<std::string>& bus_names, const std::vector<std::string>& stop_names) const;
