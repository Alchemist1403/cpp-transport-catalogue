...
void TransportRouter::BuildGraph() {
    ...
    graph_ = std::make_unique<Graph>(stop_to_id.size());
        ...
        if (bus.is_roundtrip) {
            ...
            for (size_t i = 0; i < n; ++i) {
                ...
                for (size_t span = 1; span <= max_span; ++span) {
                    size_t seg_from = (i + span - 1) % n;
                    size_t seg_to = (i + span) % n;
                    int segment_distance = catalogue_.GetDistance(bus.stops[seg_from], bus.stops[seg_to]);
                    ...
                    edge.from = stop_to_id.at(bus.stops[i]->name);
                    edge.to = stop_to_id.at(bus.stops[(i + span) % n]->name);
                    edge.weight = travel_time;
                    
                    graph::EdgeId edge_id = graph_->AddEdge(edge);
                    edge_info_[edge_id] = {bus.name, static_cast<int>(span)};
                }
            }
        } else {
            for (size_t i = 0; i < bus.stops.size(); ++i) {
                ...
                for (size_t j = i + 1; j < bus.stops.size(); ++j) {
                    int segment_distance = catalogue_.GetDistance(bus.stops[j - 1], bus.stops[j]);
                    ...
                    graph::Edge<double> edge;
                    edge.from = stop_to_id.at(bus.stops[i]->name);
                    edge.to = stop_to_id.at(bus.stops[j]->name);
                    edge.weight = travel_time;
                    
                    graph::EdgeId edge_id = graph_->AddEdge(edge);
                    edge_info_[edge_id] = {bus.name, static_cast<int>(j - i)};
                }
            }
            ...
            for (size_t i = 1; i < bus.stops.size(); ++i) {
                ...
                for (size_t j = i; j > 0; --j) {
                    int segment_distance = catalogue_.GetDistance(bus.stops[j], bus.stops[j - 1]);
                    ...
                    graph::Edge<double> edge;
                    edge.from = stop_to_id.at(bus.stops[i]->name);
                    edge.to = stop_to_id.at(bus.stops[j - 1]->name);
                    edge.weight = travel_time;
                    
                    graph::EdgeId edge_id = graph_->AddEdge(edge);
                    edge_info_[edge_id] = {bus.name, static_cast<int>(i - j + 1)};
                }
            }
        }
    }
...
Route TransportRouter::BuildRoute(const std::string& from_stop, const std::string& to_stop) const {
    Route result;
    
    auto from_it = stop_name_to_vertex_.find(from_stop);
    auto to_it = stop_name_to_vertex_.find(to_stop);
